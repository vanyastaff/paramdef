...
impl dyn Node {
    // No additional methods needed - Visibility trait methods are available
    // automatically through the supertrait bound
}

/// Base trait for all node types with visibility support.
///
/// When the `visibility` feature is enabled, all nodes automatically
/// implement visibility methods.
#[cfg(feature = "visibility")]
pub trait Node: Send + Sync + Debug + crate::types::traits::Visibility {
    /// Returns the node's metadata.
    fn metadata(&self) -> &Metadata;
